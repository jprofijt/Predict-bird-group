...
boxplot(BirdBones[length], 
        main = "Boxplot of bird Bone lengths",
        xlab = "Bone",
        ylab = "lenght in mm")
boxplot(BirdBones[diameter],
        main = "Boxplots of Bird Bone diameters",
        xlab = "bone",
        ylab = "diameter in mm")
```
...
As you can see there are quite a few outliers in all groups except in group R, The raptors. 
```{r}
library(ggplot2)
ggplot(BirdBones.noNA, aes(x = log(length.mean), y = log(diameter.mean))) +
  geom_point()+
  geom_smooth(method = lm)+
  ggtitle("Corelation between bone diameter & bone length")+
